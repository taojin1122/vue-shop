<template>
  <div>
      ...
      >
        ...
        >
          ...
          <el-tab-pane label="商品属性" name="2">
            <el-form-item
              :label="item.attr_name"
              v-for="(item, i) in onlyParamsDate"
              :key="i"
            >
    ...
    <!-- 图片预览对话框 -->
    <el-dialog title="图片预览" :visible.sync="showPictureVisible" width="50%">
...
export default {
  ...
  methods: {
    ...
    beforeTabLeave(activeName, oldActiveName) {
      console.log('即将离开的tab标签页：' + oldActiveName)
      console.log('即将进入的tab标签页：' + activeName)
    ...
    tabClicked() {
      ...
      if (this.activeIndex === '1') {
        ...
        this.getOnlyParamsData()
      }
    },
    async getOnlyParamsData() {
      ...
      console.log(this.onlyParamsDate)
    },
    async getParamsList() {
      const { data: res } = await this.$http.get(
        `categories/${this.catId}/attributes`,
